...
import {ArrowPathIcon, DownloadIcon, SparklesIcon, FileImageIcon, PlusIcon, ArrowRightIcon, PlayIcon, XMarkIcon} from './icons';
// @ts-ignore
...
interface VideoResultProps {
  ...
  draftSegment: StorySegment | null;
  onRetry: () => void;
  onNewVideo: () => void;
  onAddScene: () => void;
  onExtend: () => void;
  ...
  return (
    <div className="w-full flex flex-col items-center gap-6">
      ...
      <div className="w-full relative flex flex-col items-center gap-6 p-8 bg-gray-800/50 rounded-lg border border-gray-700 shadow-2xl overflow-visible">
        ...
        {/* Play Full Story Button - Top Right (Only when not in draft mode and has scenes) */}
        {!isDraft && timeline.length > 1 && (
          <button
          ...
          <h2 className="text-2xl font-bold text-gray-200">
             {isDraft 
               ...
               : (isPlayingStory 
                   ...
                   : (timeline.length > 0 ? `Scene ${timeline.length} Saved` : 'Scene Ready')
                 )
             }
          </h2>
          {isDraft && <p className="text-sm text-indigo-300 mt-1 animate-pulse">Select "Keep" to add to your story</p>}
        </div>
        ...
        >
          <video
            ...
            autoPlay={isPlayingStory || isDraft}
            onEnded={handleVideoEnded}
            loop={isDraft} // Loop draft for review
            ...
            {isDraft ? (
              <>
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-red-900/50 hover:bg-red-800 text-red-200 font-semibold rounded-lg transition-all active:scale-95 border border-red-800"
                  title="Discard this scene and try again">
                ...
                <button
                  onClick={onKeepScene}
                  className="flex items-center gap-2 px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg transition-all active:scale-95 shadow-lg shadow-indigo-900/40 ring-2 ring-indigo-500/20"
                  title="Save this scene to the timeline">
      ...
      {/* Timeline View (Always show confirmed history) */}
      {timeline.length > 0 && (
         <div className="w-full max-w-4xl p-4 bg-[#1a1a1a] rounded-xl border border-gray-800 overflow-x-auto">
            ...
            <div className="flex gap-4 min-w-min">
              ...
              {/* Draft Placeholder */}
              {isDraft && (
                 <div className="relative flex-shrink-0 w-48 rounded-lg overflow-hidden bg-gray-800/50 border-2 border-dashed border-indigo-500/50 flex items-center justify-center">
                    <span className="text-xs text-indigo-400 animate-pulse font-medium">Reviewing...</span>
                 </div>
              )}
            </div>
         </div>
